...
class PulsarService(private val context: Context, private val bAddress: String) {
    ...
    suspend fun connect(): Boolean {
        ...
        try {
            val device: BluetoothDevice? =
                BluetoothAdapter.getDefaultAdapter().getRemoteDevice(bAddress)
            bluetoothSocket =
                device?.createInsecureRfcommSocketToServiceRecord(UUID.fromString(Constants.UUID))
            ...
            val res = tryAttempts(
                injectCRC(
                    if (i < 2)
                        ...
                        splitAddressPulsar(
                            (resultDict["address"] ?: 0).toString()
                        ) + Constants.ADDRESS_REQUEST[i]
                )
            )
            if (res.first == 1) {
    ...
    private suspend fun writeNewValue(
        address: ByteArray,
        newValue: Double,
        devId: Int
    ): Triple<Int, ByteArray, Double> {
        ...
        val res = tryAttempts(
            injectCRC(
                address + Constants.WRITE_VALUE_REQUEST + dataValue + toBytes(
                    1,
                    2
                )
            )
    ...
    private suspend fun writeAddress(
        address: ByteArray,
        newAddress: Int,
        devId: Int
    ): Triple<Int, ByteArray, Int> {
    ...
    object Constants {
        ...
        val TVH_DEVICE = setOf(
            297 to "Пульсар модуль счетчика воды v6",
        ...
        val ADDRESS_REQUEST = arrayOf(
            byteArrayOf(
                0xf0.toByte(), 0x0f, 0x0f, 0xf0.toByte(), 0x00,
                0x00, 0x00, 0x00, 0x00
            ),
            byteArrayOf(
                0x00, 0x00, 0x00, 0x00,
        ...
        val WRITE_VALUE_REQUEST = byteArrayOf(
            0x03, 0x12,
            0x01, 0x00, 0x00, 0x00
        )
